...
<form method="post">
	...
	<?php
		$sql = "select t1.*,if(status=0,'Pending',if(status=1,'Approved','Reject')) from registration t1,login t2 where t1.mail = t2.username";
		$tbl = getDatas($sql);
...
?>
<td><a href="?id=<?php echo $tbl[$i][1]?>&mode=pen">Pending</a></td>
<td><a href="?id=<?php echo $tbl[$i][1]?>&mode=app">Approved</a></td>
<td><a href="?id=<?php echo $tbl[$i][1]?>&mode=rej">Rejected</a></td>
<?php
			echo "</tr>";
		}
	?>
